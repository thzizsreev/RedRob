...
INSTRUCTOR-large is a T5-based model. The project exports **only the T5 encoder** to ONNX. Instruction masking, mean pooling over non-instruction tokens, dense projection, and L2 normalization are implemented in Python at inference time (`tracks/instructor/onnx_embedder.py`). See `docs/instructor-large-onnx-setup.md` for background.
...
| `docs/track_a_architecture_migration.md` | 2304-d 3-block design rationale |
| `docs/instructor-large-onnx-setup.md` | Why auto ONNX export fails; manual T5 export |
| `docs/stage1_clustering_filtering_implementation_guide.md` | UMAP + HDBSCAN + filter algorithm details |
| `docs/onnx_instructor_implementation.md` | Alternate sentence-transformers ONNX path (not used by current code) |
