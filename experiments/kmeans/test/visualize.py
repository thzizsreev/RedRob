"""Plotly scatter plots for cluster and landmark views."""
...
def _cluster_colors(labels: np.ndarray) -> tuple[list[str], dict[str, str]]:
    ...
    color_map = {
        label: palette[i % len(palette)] for i, label in enumerate(unique)
    }
...
def write_cluster_plot(
    ...
    *,
    ...
    fig = _build_base_figure(
        ...
        records,
        labels,
        title="UMAP 2D — K-Means clusters",
    )
...
def write_landmark_plot(
    ...
    *,
    ...
    fig = _build_base_figure(
        ...
        records,
        labels,
        title="UMAP 2D — K-Means landmarks highlighted",
        marker_size=8,
