...
from paths import PHASE_A_CSV, PHASE_B_CSV  # noqa: E402
...
def phase_a_passed(output_dir: Path) -> tuple[bool, str | None]:
...
def run_phase_b(config_path: Path, output_dir: Path) -> list[dict[str, Any]]:
    ...
    csv_path = output_dir / PHASE_B_CSV
...
if __name__ == "__main__":
    ...
    ok, reason = phase_a_passed(_output)
    if not ok:
        print(f"Phase B skipped: {reason}")
        print("Run run_test.py with FORCE_PHASE_B = True to override.")
        raise SystemExit(1)
    run_phase_b(_config, _output)
