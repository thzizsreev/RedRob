"""CPU ONNX embedder for sandbox / CPU-only precompute (no CUDA requirement)."""
...
def _create_cpu_session(onnx_path: Path) -> ort.InferenceSession:
...
class InstructorONNXCPU:
    """Same encode API as production embedder; CPU-only."""
    ...
    def __init__(
        ...
        self.session = _create_cpu_session(onnx_path)
...
def load_cpu_embedder() -> InstructorONNXCPU:
    print("Loading INSTRUCTOR-large ONNX embedder (CPU)...")
    return InstructorONNXCPU()
