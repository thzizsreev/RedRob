...
def build_union(
    ...
    """Union candidate IDs from L1, L2, L4 with miss-penalty ranks."""
    ids: set[str] = set()
    for frame in (l1, l2, l4):
        if frame.height > 0:
    ...
    dense_miss = config.per_query_k_dense + 1
    sparse_miss = config.per_query_k_sparse + 1

    union = union.join(l1, on="candidate_id", how="left")
    union = union.join(l2, on="candidate_id", how="left")
    union = union.join(l4, on="candidate_id", how="left")

    union = union.with_columns(
        pl.col("q1_rank").fill_null(dense_miss).cast(pl.Int64),
        pl.col("q2_rank").fill_null(dense_miss).cast(pl.Int64),
        pl.col("bm25_rank").fill_null(sparse_miss).cast(pl.Int64),
    )
    return union
