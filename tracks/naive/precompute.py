...
from tracks.naive.config import ARTIFACTS_DIR, SAMPLE1K_PATH
from tracks.naive.encode import encode_passages, load_encoder
from tracks.naive.passage import build_passage
from tracks.naive.store import save_index


def load_sample_candidates(path: Path) -> list[dict]:
...
def run_precompute() -> None:
    print("=" * 60)
    print("NAIVE PRECOMPUTE — building resume vector index")
    print("=" * 60)

    records = load_sample_candidates(path=SAMPLE1K_PATH)
    ...
    print("\n" + "=" * 60)
    print("Naive precompute complete.")
    print("=" * 60)
